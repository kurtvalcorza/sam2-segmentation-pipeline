...
|---|---|---|---|---|---|---|---|
| `sam2_segmentation_colab.ipynb` | `E2E` | `GUIDED` | standalone (generated) | 24 deterministic labelled scenes; 18/6 train/held-out split; pinned pretrained baseline; two real gradient epochs on the first mask-token hypernetwork; held-out mask IoU and prompt-box baseline; unseen-scene prediction; SafeTensors adapter export; fresh pinned-base reload equivalence | CUDA required; Colab T4 or Kaggle T4 supported | ZIP containing paired `images/` and `masks/` files, off by default | **Candidate** — source and an 11/11-cell local CUDA pre-flight pass exist; exact committed notebook blob still needs a clean supported-runtime `Run all` |
